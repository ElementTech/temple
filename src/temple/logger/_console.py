...
class ConsoleLogger:
    ...
    def title(self, name, num_requests, repeat_index=None):
        ...
        print(f"{name_text}Sending {num_requests} requests" f"{self._repeat_text(repeat_index)}:\n")

    def _get_indicator_text(self, request=None, assertion=None):
        color, symbol = get_indicator(request.state if request else get_assertion_status(assertion))
    ...
    def _get_response_code_text(self, request):
        ...
        if request.state == RequestState.NOT_RAISED:
            not_raised_text = self.color(" HTTP status code ignored.", "grey")
    ...
    def _headers_text(self, response):
        return "\n".join(f"{self.bold(key)}: {value}" for key, value in response.headers.items())
    ...
    def start_request(self, request):
        ...
        self._active = Thread(target=_print_spinner_and_text, args=[text, self._stop_event])
