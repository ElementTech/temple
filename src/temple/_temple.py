...
class Temple:
    def __init__(self, plan_dict, options_override=None, variables_override=None):
        ...
        self.options = TempleOptions(self._plan, options_override)
        self.variables = {**self._plan.get("variables", {}), **variables_override}
        self.requests = self._plan.get("requests")

        if not self.requests or not isinstance(self.requests, list):
            raise AssertionError("Temple must contain requests array.")
